						...
						<?php if (!preg_match('/mobile/i', $_SERVER['HTTP_USER_AGENT']) or 1 == 1) { ?>
							<div class="panel-heading">
								<div style=" display: inline-block; vertical-align: left; text-align:right !important">
									...
									<input type="search" id="tags" enterkeyhint="done" name="search_sale_item" placeholder="Search Sale Item" class="form-control" style="display: inline-block; width: auto;" />
								...
								<div class="row">
									<div style="margin: 3px;">
										...
										<?php if (preg_match('/mobile/i', $_SERVER['HTTP_USER_AGENT'])) { ?>
											<div class="row mt-2">
												<div class="col-xs-4">
													<a href="<?php echo site_url("return_point") ?>" class="btn btn-danger btn-xs">Return Items</a>
												</div>
												<div class="col-xs-4">
													<button onclick="get_sale_reports()" data-toggle="modal" data-target="#sale_report_mode" class="btn btn-primary btn-xs">Sale Reports</button>
												</div>
												<div class="col-xs-4">
													<button onclick="get_sale_receipts()" data-toggle="modal" data-target="#item_return_modal" class="btn btn-warning btn-xs">Reprint Receipt</button>
												</div>
											</div>
										<?php } ?>
...
<script>
	...
	function update_user_item_unit_price(user_item_id) {
		...
		if (event.key === 'Enter') {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url("sale_point/update_user_item_unit_price") ?>",
				data: {
					user_item_id: user_item_id,
