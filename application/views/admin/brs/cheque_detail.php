...
<?php if ($expense) { ?>
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div style="text-align: center; border:1px solid gray; border-radius:5px; padding:5px">
                <strong>
                    <form method="post" action="<?php echo site_url(ADMIN_DIR . 'brs/change_cheque_status') ?>">
                        <input type="hidden" value="<?php echo $expense->expense_id; ?>" name="expense_id" />
                        Mark Cheque as BRS
                        <input <?php if ($expense->brs == 1) { ?> checked <?php } ?> type="radio" name="brs" value="1"> Yes
                        <span style="margin-left: 10px;"></span>
                        <input <?php if ($expense->brs == 0) { ?> checked <?php } ?> type="radio" name="brs" value="0"> No
                        <span style="margin-left: 10px;"></span>
                        <button>Change Cheque BRS Status</button>
                    </form>
                </strong>
            </div>
        </div>
