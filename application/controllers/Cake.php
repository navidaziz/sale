...
class Cake extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $table_name_singular = "expense_type";
    ...
    private function add_form($controller_name, $table_name, $table_name_singular, $primary_key, $colums)
    {
       ...
       <script>
        $(\'#' . $table_name . '\').submit(function(e) {
            ...
            $.ajax({
                type: \'POST\',
                url: \'<?php echo site_url("' . $controller_name . '/add_' . $table_name_singular . '"); ?>\', // URL to submit form data
                data: formData,
    ...
    private function list_view_code($controller_name, $table_name, $table_name_singular, $primary_key, $colums)
    {
        ...
        $list_view .= '<th>Action</th>
            ...
            <tbody>
                ...
                foreach ($rows as $row) { ?>
                    <tr>
                    <td><a href="<?php echo site_url(\'' . $controller_name . '/delete_' . $table_name . '/\' . $row->' . $primary_key . '); ?>" onclick="return confirm(\'Are you sure? you want to delete the record.\')">Delete</a> </td>
        ...
        <script>
            function get_' . $table_name_singular . '_form(' . $primary_key . ') {
                $.ajax({
                        method: "POST",
                        url: "<?php echo site_url(\'' . $controller_name . '/get_' . $table_name_singular . '_form\'); ?>",
                        data: {
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $("#datatable").DataTable({
                    ...
                    "serverSide": true,
                    "ajax": {
                        "url": "<?php echo base_url(."' . $controller_name . '/' . $table_name . '"); ?>",
                        "type": "POST"
                    },
                    "columns": [
                        {
...

        {
            ...
            "render": function(data, type, row) {
                return \'<a class="llink llink-view" href="<?php echo site_url("' . $controller_name . '/trash/"); ?>\' + row.' . $primary_key . ' + \'/\' + \'" onclick="return confirm(\'Are you sure? you want to delete the record.\')"><i class="fa fa-trash-o"></i></a><span style="margin-left: 10px;"></span>\' +
                    \'<a class="llink llink-view" href="<?php echo site_url("' . $controller_name . '/view_' . $table_name . '/"); ?>\' + row.' . $primary_key . ' + \'/\' + \'"><i class="fa fa-eye"></i></a><span style="margin-left: 10px;"></span>\' +
                    \'<a class="llink llink-edit" href="<?php echo site_url("' . $controller_name . '/edit/"); ?>\' + row.' . $primary_key . ' + \'/\' + \'"><i class="fa fa-pencil-square-o"></i></a>\';
            }
        }
    ...
    private function controller_code($controller_name, $table_name, $table_name_singular, $primary_key, $colums)
    {
        ...
        $input = $this->get_inputs();
            ...
            $this->load->view("' . $controller_name . '/get_' . $table_name_singular . '_form", $this->data);
            }';
        ...
        ';
        $controller .= ' $inputs["created_by"] = $this->session->userdata("user_id");
        $' . $primary_key . ' = (int) $this->input->post("' . $primary_key . '");
                if ($' . $primary_key . ' == 0) {
